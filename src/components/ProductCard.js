...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="mx-auto relative w-[290px] md:min-w-[310px] lg:min-w-[250px] lg:w-auto xl:max-w-[300px]">
      ...
      {(() => {
        const isOutOfStock = Number(product.total_variants_quantities) === 0;
          ...
          <>
            <div className="relative w-full h-[270px] xl:w-full xl:h-[300px] rounded-3xl xl:rounded-t-3xl rounded-b-none overflow-hidden bg-[#F0EEED]">
              <Image
                ...
                className={`object-contain bg-[#F0EEED] transition-transform duration-300 ease-in-out ${
                  !isOutOfStock
                    ? "group-hover:scale-105"
                    : "opacity-50 grayscale"
                }`}
              />
              {/* Only show Sale tag if in stock */}
              {!isOutOfStock && salePrice !== null && (
              ...
              {/* Optional: Overlay for Out of Stock image */}
              {isOutOfStock && (
                <span className="absolute top-3 left-3 bg-gray-500 text-white text-xs px-2 py-1 rounded-lg shadow-md font-semibold z-10">
                  Sold Out
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col justify-between px-4 gap-3 pb-4">
              <p
                className={`text-[16px] lg:text-[18px] font-bold lg:pt-0 ${
        ...
        return isOutOfStock ? (
          <div className="group flex flex-col rounded-3xl xl:h-[380px] gap-4 overflow-hidden border border-gray-200 cursor-not-allowed opacity-75">
            {CardContent}
          </div>
        ) : (
          <Link
            href={`/products/${product.slug}`}
            className="group flex flex-col rounded-3xl xl:h-[380px] gap-4 overflow-hidden hover:shadow-[0_0_5px_rgba(0,0,0,0.1)] border border-gray-300/60 transition-shadow"
          >
            {CardContent}
          </Link>
        );
